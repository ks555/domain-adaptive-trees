...
class DecisionTreeClassifier(object):
    ...
    def build(self, X, y, depth):
        ...
        if depth >= self.max_depth:  # base case 4: max depth reached
            return {'type': 'leaf', 'val': stats.mode(y).mode[0], 'tot': leny,
                    'dist': y.value_counts(sort=False) / leny}, 0
        # Recursively generate trees!
        # find one split given an information gain todo
    ...
    def find_best_split_attribute(self, x, y, is_cat):
        ...
        for value in values:
            ...
            if n_right < self.min_cases:
                continue
            # todo: should here what's the current path in my tree: use self.
            entropy_left = self.info(y[cond])  # < value todo: da goes here| need to know current path here self.subtree={}
